...
const (
	...
	// the completion status the SDK requires for transfers retired by an
	// EndpointPause command rather than by actual completion.
...
func darwinOpenUSBHostDevice(registryID uint64, capture bool) (*darwinUSBHostDevice, error) {
	...
	var errorPtr *C.char
	handle := C.box_usbhost_device_open(C.uint64_t(registryID), C.bool(capture), &info, &status, &errorPtr)
	if handle == nil {
		err := darwinCError(errorPtr)
		switch int32(status) {
		case int32(C.kIOReturnNotPermitted), int32(C.kIOReturnNotPrivileged):
			err = E.Cause1(err, os.ErrPermission)
		}
		return nil, err
	}
...
func darwinDeviceInfoFromC(info *C.box_usbhost_device_info_t) darwinUSBHostDeviceInfo {
	...
	product := cCharArrayString(unsafe.Pointer(&info.product[0]))
