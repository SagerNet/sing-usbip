...
func shutdownUSBIPHostDevice(busid string) {
	...
	if driverErr == nil && driver == "usbip-host" {
...
func startRealUSBIPServer(t *testing.T, devices []DeviceMatch) (*ServerService, M.Socksaddr) {
	t.Helper()
	requireUSBIPHost(t)

	server, err := NewServerService(context.Background(), ServerOptions{
		Logger:        newTestLogger(t),
		ListenAddress: M.ParseSocksaddrHostPort("127.0.0.1", pickFreeTCPPort(t)),
		Devices:       devices,
	})
	require.NoError(t, err)
	...
	return server, M.SocksaddrFromNet(server.ListenAddr())
}

func startRealUSBIPClient(t *testing.T, destination M.Socksaddr, devices []DeviceMatch) *ClientService {
	t.Helper()
	requireVHCI(t)

	client, err := NewClientService(context.Background(), ClientOptions{
		Logger:        newTestLogger(t),
...
func TestUSBIPInteropOurServerWithOfficialClientACM(t *testing.T) {
	...
	server, address := startRealUSBIPServer(t, []DeviceMatch{{Serial: gadget.serial}})
...
func TestUSBIPControlHotplugACMReattach(t *testing.T) {
	...
	server, address := startRealUSBIPServer(t, []DeviceMatch{{
		VendorID:  testVendorID,
		ProductID: testACMProductID,
	}})
	client := startRealUSBIPClient(t, address, []DeviceMatch{{
		VendorID:  testVendorID,
		ProductID: testACMProductID,
	}})
	_ = client
...
func TestUSBIPControlImportAllACMAndHID(t *testing.T) {
	...
	server, address := startRealUSBIPServer(t, []DeviceMatch{
		{VendorID: testVendorID, ProductID: testACMProductID},
		{VendorID: testVendorID, ProductID: testHIDProductID},
	})
...
func dialDevListServer(address M.Socksaddr) (net.Conn, error) {
...
func fetchControlDevListOnce(address M.Socksaddr) ([]DeviceEntry, error) {
	conn, err := dialDevListServer(address)
	if err != nil {
		return nil, err
	}
	defer conn.Close()
	return FetchControlDeviceEntries(conn)
}

func fetchStandardDevListOnce(address M.Socksaddr) ([]DeviceEntry, error) {
	conn, err := dialDevListServer(address)
	if err != nil {
		return nil, err
	}
	...
	if header.Version != ProtocolVersion || header.Code != OpRepDevList || header.Status != OpStatusOK {
		return nil, E.New("unexpected OP_REP_DEVLIST reply version=", fmt.Sprintf("0x%04x", header.Version),
			" code=", fmt.Sprintf("0x%04x", header.Code), " status=", header.Status)
	}
	return ReadOpRepDevListBody(conn)
}

func findDeviceEntryByBusID(entries []DeviceEntry, busid string) (DeviceEntry, bool) {
	for i := range entries {
		if entries[i].Info.BusIDString() == busid {
...
func waitForDevListEntry(t *testing.T, fetch func(M.Socksaddr) ([]DeviceEntry, error), address M.Socksaddr, busid string) DeviceEntry {
	t.Helper()
	var entry DeviceEntry
	require.Eventually(t, func() bool {
		entries, err := fetch(address)
		if err != nil {
			return false
		}
		found := false
		entry, found = findDeviceEntryByBusID(entries, busid)
		return found
	}, 30*time.Second, 500*time.Millisecond)
	return entry
}

// Capturing a device rewrites its sysfs descriptor view (rebinding to
// usbip-host deconfigures it), and the server refreshes its snapshot on a
// later reconcile, so two independently timed fetches can observe different
// descriptor generations. Fetch both protocols in the same tick and retry
// until they agree.
func waitForMatchingDevListEntries(t *testing.T, address M.Socksaddr, busid string) (DeviceEntry, DeviceEntry) {
	t.Helper()
	var controlEntry, standardEntry DeviceEntry
	require.Eventually(t, func() bool {
		controlEntries, err := fetchControlDevListOnce(address)
		if err != nil {
			return false
		}
		foundControl, found := findDeviceEntryByBusID(controlEntries, busid)
		if !found {
			return false
		}
		standardEntries, err := fetchStandardDevListOnce(address)
		if err != nil {
			return false
		}
		foundStandard, standardFound := findDeviceEntryByBusID(standardEntries, busid)
		if !standardFound || foundControl.Info != foundStandard.Info {
			return false
		}
		controlEntry = foundControl
		standardEntry = foundStandard
		return true
	}, 30*time.Second, 500*time.Millisecond)
	return controlEntry, standardEntry
}

func TestUSBIPControlListCarriesDeviceIdentity(t *testing.T) {
	requireRoot(t)
	resetUSBIPInteropState(t)
	...
	controlEntry, standardEntry := waitForMatchingDevListEntries(t, address, gadget.busid)
	require.Equal(t, "Codex USBIP ACM", controlEntry.Product)
	require.Equal(t, gadget.serial, controlEntry.Serial)
	require.Equal(t, testVendorID, controlEntry.Info.IDVendor)
	require.Equal(t, testACMProductID, controlEntry.Info.IDProduct)
	require.Equal(t, filepath.Join(sysBusUSBDevices, gadget.busid), controlEntry.Info.PathString())

	require.Equal(t, "", standardEntry.Serial)
	require.Equal(t, "", standardEntry.Product)

	pathEnd := len(standardEntry.Info.PathString())
	require.Equal(t,
		bytes.Repeat([]byte{0}, len(standardEntry.Info.Path)-pathEnd),
		standardEntry.Info.Path[pathEnd:])

	require.NoError(t, server.Close())
}

func TestUSBIPControlListUnsupportedByOfficialServer(t *testing.T) {
	requireRoot(t)
	resetUSBIPInteropState(t)
	tools := requireUSBIPTools(t)
	...
	address := M.ParseSocksaddrHostPort("127.0.0.1", port)

	_, err := fetchControlDevListOnce(address)
	require.Error(t, err)

	entry := waitForDevListEntry(t, fetchStandardDevListOnce, address, gadget.busid)
	require.Equal(t, "", entry.Serial)
	require.Equal(t, "", entry.Product)
	require.Equal(t, testVendorID, entry.Info.IDVendor)
	require.Equal(t, testACMProductID, entry.Info.IDProduct)
}
