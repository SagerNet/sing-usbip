...
static void box_set_error_from_nserror(char **error_out, NSString *prefix, NSError *error) {
	...
	if (error.localizedFailureReason != nil) {
		description = [NSString stringWithFormat:@"%@ %@", description, error.localizedFailureReason];
	}
	box_set_error_string(error_out, [NSString stringWithFormat:@"%@: %@ (%@ 0x%x)", prefix, description, error.domain, (uint32_t)error.code]);
}
...
static void box_fill_registry_info(io_service_t service, box_usbhost_device_info_t *info) {
	...
	box_string_property(service, @kUSBHostDevicePropertyProductString, info->product, sizeof(info->product));
	box_fill_busid(info);
...
box_usbhost_device_watcher_t *box_usbhost_device_watcher_create(uintptr_t ref, char **error_out) {
	@autoreleasepool {
		...
		dispatch_queue_t queue = dispatch_queue_create("io.nekohasekai.sing-usbip.usbhost-watch", DISPATCH_QUEUE_SERIAL);
...
// Teardown order matters: destroying the notification port stops new
// callback dispatches, the queue drain completes any callback already
// enqueued, and only then is it safe to release the iterators those
// callbacks call IOIteratorNext on. Releasing the iterators first lets
// a queued callback run against a released handle.
...
box_usbhost_device_t *box_usbhost_device_open(uint64_t registry_id, bool capture, box_usbhost_device_info_t *info_out, int32_t *status_out, char **error_out) {
	@autoreleasepool {
		if (status_out != NULL) {
			*status_out = 0;
		}
		...
		if (host_device == nil) {
			if (status_out != NULL) {
				*status_out = (int32_t)(error != nil ? error.code : kIOReturnError);
			}
...
bool box_usbhost_device_iso(box_usbhost_device_t *device, uint8_t endpoint, uint8_t *data, size_t data_len, int32_t start_frame, bool asap, box_usbhost_iso_packet_t *packets, size_t packet_count, size_t *actual_out, int32_t *status_out, char **error_out) {
	...
	@autoreleasepool {
		...
		for (size_t i = 0; i < packet_count; i++) {
			...
			// Per-frame status must pass through even when the overall
			// request succeeds: an importer's class driver relies on
			// individual frame errors (CRC, underrun) that do not fail
			// the whole URB.
			packets[i].status = (int32_t)transactions[i].status;
...
box_usbhost_controller_t *box_usbhost_controller_create(uintptr_t ref, uint8_t port_count, uint32_t speed, char **error_out) {
	@autoreleasepool {
		...
		dispatch_queue_t queue = dispatch_queue_create("io.nekohasekai.sing-usbip.usbhost-controller", DISPATCH_QUEUE_SERIAL);
		NSError *error = nil;
