...
func TestDarwinUSBIPClientSmoke(t *testing.T) {
	...
	client, err := NewClientService(context.Background(), ClientOptions{
		...
		ServerAddress: fakeServer.address,
		Devices:       []DeviceMatch{{BusID: darwinFakeBusID}},
	})
	require.NoError(t, err)
	defer func() {
		fakeServer.Close()
...
func TestDarwinUSBIPServerSmoke(t *testing.T) {
	...
	server, err := NewServerService(context.Background(), ServerOptions{
		...
		Devices:       []DeviceMatch{{BusID: candidate.key.BusID}},
	})
	require.NoError(t, err)
	defer server.Close()
	err = server.Start()
	if err != nil {
		t.Skipf("IOUSBHostDevice unavailable: %v", err)
	}
	if len(server.ledger.AvailableExports()) == 0 {
		t.Skipf("IOUSBHostDevice capture unavailable for %s", candidate.key.BusID)
	}

	destination := M.SocksaddrFromNet(server.ListenAddr())
